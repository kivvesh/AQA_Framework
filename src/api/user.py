...
class User(BaseApi):
    ...
    def get_user(self,id_user:int,status=200,*args,**kwargs) -> dict | Response:
        ...
        if status==200:
            ...
            try:
                ...
                assert False,f'Некорректный ответ по пользователю {response.json().get("data")} {error}'
        else:
            return response
    ...
    def post_user(self,*args,**kwargs):
        ...
        response = self.request.post(url=f'{self.base_url}{endpoint}', headers=headers, data=json.dumps(kwargs))
        try:
            ResponsePostUser(**response.json())
            return response.json()
        except ValidationError as error:
            assert False, f'Некорректный ответ по созданию пользователя {response.json()} {error}'
    ...
    def delete_user(self,user_id:int):
        ...
        response = self.request.delete(url=f'{self.base_url}{endpoint}', headers=headers)
        assert response.status_code == 204, f'Некорректный статус запроса {response.status_code}'
    ...
    def post_register_user(self,status=200,*args,**kwargs):
        ...
        response = self.request.post(url=f'{self.base_url}{endpoint}', headers=headers, data=json.dumps(kwargs))
        assert response.status_code == status, f'Некорректный статус запроса {response.status_code}'
